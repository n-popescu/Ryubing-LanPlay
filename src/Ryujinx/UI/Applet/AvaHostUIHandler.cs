        ...
        public bool DisplayCabinetDialog(out string userText)
        {
            ...
            Dispatcher.UIThread.InvokeAsync(async () =>
            {
                try
                {
                    ...
                    (UserResult result, string userInput) =
                        await SwkbdAppletDialog.ShowInputDialog(LocaleManager.Instance[LocaleKeys.Dialog_Amiibo_CabinetDialog], args);
                    if (result == UserResult.Ok)
                    {
                        inputText = userInput;
                        okPressed = true;
        ...
        public void DisplayCabinetMessageDialog()
        {
            ...
            Dispatcher.UIThread.InvokeAsync(async () =>
            {
                ...
                await ContentDialogHelper.CreateInfoDialog(
                    LocaleManager.Instance[LocaleKeys.Dialog_Amiibo_CabinetScanDialog],
                    string.Empty,
                    LocaleManager.Instance[LocaleKeys.InputDialogOk],
                    string.Empty,
                    LocaleManager.Instance[LocaleKeys.Dialog_Amiibo_CabinetTitle]
                );
            });
            dialogCloseEvent.WaitOne();
        }
