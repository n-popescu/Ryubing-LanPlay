...
namespace Ryujinx.HLE.HOS.Services.Account.Acc.SwitchNet
{
    ...
    static class SwitchNetAccountSession
    {
        ...
        /// </summary>
        /// <remarks>
        /// Missing settings are not an error: SwitchNet support can be on for the DNS
        /// redirect while the operator has not set up an account, and that is a working
        /// configuration for anything that does not need a game-server identity.
        /// </remarks>
        public static bool TryGetClient(HleConfiguration configuration, out SwitchNetAccountClient client)
        {
            client = null;

            if (configuration == null || !configuration.EnableSwitchNet)
            {
                return false;
            }
